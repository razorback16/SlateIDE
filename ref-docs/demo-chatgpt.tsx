...
const Example = () => {
  ...
  const streamMessage = useCallback(
    async (message: MessageType) => {
      ...
      // Stream the response
      await streamResponse(newMessage.key, firstVersion.id, firstVersion.content, message.reasoning)
    },
    [streamResponse]
  )
  ...
  const addUserMessage = useCallback(
    (content: string) => {
      ...
      setTimeout(() => {
        ...
        streamResponse(assistantMessageKey, assistantMessageId, randomResponse, reasoning)
      }, 500)
    },
    [streamResponse]
  )
